          ...
          <div class="order-content">
            ...
            <div class="orders">
              ...
              >
                ...
                <tbody>
                  <!-- 
                    第一次加载页面时，orderList是空对象，所以不会渲染
                    等到页面挂载成功，拿到数据了，orderList不是空对象了，就能开始渲染了

                    如果出现多行，只有第一个td需要循环
                   -->
                  <tr
                    v-for="(detail, index) in record.orderDetailList"
                    :key="detail.id"
                  >
                    <td width="60%">
                    ...
                    <td
                      ...
                      width="8%"
                      :class="{ center: true, hide: index != 0 }"
                    >
                      {{ record.consignee || "法外狂徒张三" }}
                    </td>
                    <td
                      :rowspan="record.orderDetailList.length"
                      width="13%"
...
export default {
  ...
  async mounted() {
    /* 
      ...
      将数据渲染到页面中
    */
    this.orderList = await reqGetOrderList({ page: 1, limit: 5 });
  },
  methods: {
    // 点击页码，显示对应页的数据
    async toPage(page) {
